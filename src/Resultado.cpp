...
Resultado::~Resultado() {
    // Los jugadores son propiedad del Equipo, no se eliminan aqui
}
...
void Resultado::actualizarHistoricos(Equipo* e1, Equipo* e2) {
    int resE1, resE2;
    if (gfEquipo1 > gfEquipo2)      { resE1 =  1; resE2 = -1; }
    else if (gfEquipo1 < gfEquipo2) { resE1 = -1; resE2 =  1; }
    else                            { resE1 =  0; resE2 =  0; }
    ...
    for (int i = 0; i < 11; i++) {
        if (convocados1[i] != nullptr)
            convocados1[i]->actualizarEstadisticas(0, minutos, 0, 0, 0);
        if (convocados2[i] != nullptr)
            convocados2[i]->actualizarEstadisticas(0, minutos, 0, 0, 0);
    }
}
